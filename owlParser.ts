...
class Relation {
    ...
    populateGrammarLine(): string {
        return this.populateName() + `: '{' '${this.name}' '${this.output}' ${this.output}ID '}';`;
    }
}
...
export default function parseOwl(data: any): string {
    ...
    declarations.forEach((element: any) => {
        ...
        element[name].forEach((subclass: any) => {
            let result = currentMap.get(Object.keys(subclass['$'])[0])(
                subclass['$'][Object.keys(subclass['$'])[0]],
                false,
            );
    ...
    return populateGrammar([...dec.values()]);
}
